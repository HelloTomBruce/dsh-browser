...
export async function waitForLoginTool(config: BrowserConfig, args: Record<string, any>) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (successSelector) {
        const found = await page.$(successSelector).catch(() => null);
        if (found !== null) { loggedIn = true; break; }
      } else if (successUrl) {
        try {
          if (new RegExp(successUrl).test(page.url())) { loggedIn = true; break; }
        } catch {
          /* 非法正则,忽略 */
        }
      } else if (page.url() !== startUrl && page.url() !== "about:blank") {
        loggedIn = true; // 默认:URL 跳转视为登录完成
        break;
      }
    } catch {
      break;
  ...
  return { loggedIn, ...(await pageIdentity(page)) };
}
