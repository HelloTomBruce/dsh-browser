...
import { openTool, snapshotTool, clickTool, typeTool, pressTool, evalTool, screenshotTool, waitTool, backTool, reloadTool, statusTool, closeTool, tabsTool, downloadTool, uploadTool, cookiesTool, formTool, formSaveTool, formsTool, profileTool, elementsTool, waitForLoginTool, waitForTool, assertTool, networkTool, recordTool, replayTool, } from "./tools.js";
export function registerTools(ctx, config) {
    ...
    const stepImpls = {
        ...
        browser_assert: (a) => assertTool(config, a, undefined),
    ...
    register(definePlainTool({
        ...
        render: (_args, value) => {
            ...
            if (value.set)
                return `Cookie ${value.name} set for ${value.url}`;
    ...
    register(definePlainTool({
        ...
        render: (args, value) => {
            if (args.action === "delete")
                return value.deleted ? `Deleted "${value.name}"` : `No form "${value.name}"`;
            return (value.forms ?? [])
    ...
    register(definePlainTool({
        ...
        render: (args, value) => {
            if (args.action === "use")
                return `Switched to profile "${value.current}"`;
    ...
    register(definePlainTool({
        ...
        render: (_args, value) => (value.elements ?? [])
            .map((e) => `<${e.tag}${e.type ? ` type=${e.type}` : ""}>${e.name ? ` name=${e.name}` : ""}${e.id ? ` id=${e.id}` : ""}${e.placeholder ? ` ph="${e.placeholder}"` : ""}${e.text ? ` "${e.text.slice(0, 40)}"` : ""}${e.selector ? ` → ${e.selector}` : ""}`)
    ...
    register(definePlainTool({
        ...
        render: (args, value) => {
            if (args.action === "start")
                return "开始录制(后续浏览器调用将记录)";
            if (args.action === "stop")
                return `录制结束,共 ${value.steps} 步(browser_record save <name> 保存)`;
            if (args.action === "save")
                return `已保存录制 "${value.saved}"(${value.steps} 步)`;
            if (args.action === "delete")
                return value.deleted ? `已删除 "${value.name}"` : `无此录制 "${value.name}"`;
