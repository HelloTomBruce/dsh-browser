...
import { profileConfig, getProfileState, downloadsOf, launchProfile, newPage, getPage, activePage, tabList, closeBrowser, pageIdentity, stateOf, currentProfile, savedForms, lastFormFields, profileStates, setLastFormFields, setCurrentProfile, noteScreenshotDir, cleanupScreenshots, networkEntries, clearNetwork, savedRecordings, startRecording, stopRecording, testLogOf, isRecording, } from "./session.js";
import { saveRecordingFile, listRecordings, deleteRecording } from "./recordings.js";
import { workspaceCwd } from "../lib/tools.js";
export async function openTool(config, args) {
...
export async function snapshotTool(config, args) {
    const page = await getPage(config);
...
export async function clickTool(config, args) {
    const page = await getPage(config);
...
export async function typeTool(config, args) {
    const page = await getPage(config);
    const selector = String(args.selector);
...
export async function pressTool(config, args) {
    const page = await getPage(config);
...
export async function evalTool(config, args) {
    const page = await getPage(config);
...
export async function screenshotTool(config, args, exec) {
    const page = await getPage(config);
...
export async function tabsTool(config, args) {
    const action = args.action ?? "list";
    const state = stateOf();
    if (state.browser === null && state.context === null && action !== "list")
        await launchProfile(currentProfile, config);
    if (state.browser === null && state.context === null)
        return { action, tabs: [], activeId: -1 };
    switch (action) {
        case "list": {
...
export function resolveTabId(args) {
    const state = stateOf();
...
export async function downloadTool(config, args, exec) {
    ...
    if (entry === undefined)
        throw new Error(`no download at index ${index}`);
    const cwd = workspaceCwd(exec);
    ...
    if (!filePath.startsWith(resolve(dir)))
        throw new Error(`download path escapes directory: ${filePath}`);
...
export async function uploadTool(config, args, exec) {
    const page = await getPage(config);
    const cwd = workspaceCwd(exec);
...
export async function cookiesTool(config, args) {
    const page = await getPage(config);
...
export async function formTool(config, args) {
    const page = await getPage(config);
...
export async function formSaveTool(args) {
    let fields = args.fields;
    if (!Array.isArray(fields) || fields.length === 0) {
...
export async function formsTool(args) {
    const action = args.action ?? "list";
    if (action === "list") {
        return {
...
export async function profileTool(config, args) {
    const action = args.action ?? "list";
...
export async function elementsTool(config, args) {
    const page = await getPage(config);
...
export async function pollCondition(page, cond, timeoutMs, intervalMs = 500) {
    const deadline = Date.now() + timeoutMs;
    let last = { ok: false, detail: "超时" };
...
/** 通用显式等待:等待条件成立,超时抛错。 */
export async function waitForTool(config, args) {
    const page = await getPage(config);
 ...
 */
export async function waitForLoginTool(config, args) {
    const page = await getPage(config);
...
export async function assertTool(config, args, exec) {
    const page = await getPage(config);
...
function matchNetwork(entry, f) {
    if (f.urlSubstr && !entry.url.includes(String(f.urlSubstr)))
        return false;
    if (f.method && entry.method.toUpperCase() !== String(f.method).toUpperCase())
        return false;
    if (f.status !== undefined && Number(f.status) !== entry.status)
        return false;
    if (f.failedOnly === true && !entry.failed && (entry.status === undefined || entry.status < 400))
        return false;
    return true;
}
...
export async function networkTool(args) {
    ...
    if (action === "wait") {
        if (!args.url)
            throw new Error("wait 需要 url(正则字符串)");
        ...
        while (Date.now() < deadline) {
            const hit = [...entries]
                .reverse()
                .find((e) => {
                if (!new RegExp(String(args.url)).test(e.url))
                    return false;
                if (args.status !== undefined && Number(args.status) !== e.status)
                    return false;
                return true;
            });
            if (hit !== undefined)
                return { matched: hit, waitMs: Date.now() + timeoutMs - deadline - (Date.now() + timeoutMs - deadline) + 0 };
...
export async function recordTool(args) {
    ...
    if (action === "save") {
        if (isRecording())
            throw new Error("录制未结束:先 browser_record stop");
        const name = String(args.name ?? "");
        if (!name)
            throw new Error("save 需要 name");
    ...
    if (action === "delete") {
        if (!args.name)
            throw new Error("delete 需要 name");
        const name = String(args.name);
...
export async function runReplay(steps, impls, failFast) {
    const results = [];
    let stoppedAt = null;
    let passed = 0;
...
export async function replayTool(args, impls) {
    let steps = [];
    let source = "steps";
    if (typeof args.name === "string" && args.name) {
        const saved = savedRecordings.get(args.name);
        if (saved === undefined)
            throw new Error(`未找到录制: ${args.name}`);
        steps = saved.steps;
        source = args.name;
    }
